...
const mockSignals: Signal[] = [
  {
    ...
    isPremium: false,
    analysis:
      "Technical analysis for EUR/USD indicates a strong uptrend based on recent price action. Key support levels are holding firm, suggesting bullish momentum with an 85% probability of success. Market volatility is within normal ranges, and volume patterns support the upward movement. The risk-reward ratio is favorable at approximately 1:2.",
  },
  {
    id: "mock-2",
    pair: "GBP/USD",
...
export const trainModelLocally = async (symbol: string): Promise<void> => {
  try {
    ...
    const features = historicalData.map((item: any) => [item.price, item.volume || 0])
    const labels = historicalData
      .slice(1)
      .map((item: any) => (item.price > historicalData[historicalData.indexOf(item) - 1].price ? 1 : 0))
    ...
    for (const pair of pairs) {
      try {
        ...
        if (probability / 100 >= CONFIG.AI_MODEL.PREDICTION_THRESHOLD) {
          ...
          const takeProfit1 = signalType === "BUY" ? entryPrice + riskFactor : entryPrice - riskFactor
          const stopLoss = signalType === "BUY" ? entryPrice - riskFactor * 0.8 : entryPrice + riskFactor * 0.8
