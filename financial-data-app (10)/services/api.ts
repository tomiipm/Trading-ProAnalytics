import type { ForexSignal, PerformanceStats, Notification, Signal } from "@/types/forex"
...
export const fetchMarketData = async (symbol: string): Promise<any> => {
  try {
    ...
    const response = await axios.get(`${FMP_API_ENDPOINT}/quote/${formattedSymbol}?apikey=${FMP_API_KEY}`)
    ...
    if (response.data && response.data.length > 0) {
      ...
      return {
        symbol,
        price: symbol === "EUR/USD" ? 1.0825 : symbol === "GBP/USD" ? 1.265 : Math.random() * 1 + 1,
 ...
 */
export const fetchHistoricalMarketData = async (symbol: string, days = 30): Promise<any> => {
  try {
    // Format symbol for FMP API (e.g., EURUSD)
    const formattedSymbol = symbol.replace("/", "")
...
const generatePerformanceStatsFromRealData = async (): Promise<PerformanceStats> => {
  try {
    ...
    const totalPips = Math.floor(Number.parseFloat(averageProfitPerDay) * 30) // Total pips over 30 days
    ...
    const riskRewardRatio = Number.parseFloat((averageProfitPerTrade / averageLossPerTrade).toFixed(1))
