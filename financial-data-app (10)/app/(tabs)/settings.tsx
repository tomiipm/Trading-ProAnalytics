    ...
    Linking.canOpenURL(url)
      .then((supported) => {
        if (supported) {
          ...
          Alert.alert("Email Not Available", `Please contact us at ${email}`, [{ text: "OK" }])
        }
      })
      .catch((error) => {
        console.error("Error opening email:", error)
        Alert.alert("Error", `Please contact us at ${email}`, [{ text: "OK" }])
      })
  }

  const handleOpenWebsite = () => {
    Linking.openURL(CONFIG.APP.WEBSITE).catch((err) => {
